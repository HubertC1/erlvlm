...
def construct_vlm_with_key(rank=0, model_name=None):
    ...
    #     # qwen_model_name = "Qwen/Qwen2-VL-72B-Instruct-GPTQ-Int4"
    #     vlm_model = LVLMModel(model_name=model_name)
    else:
...
def query_vlm_for_summarization(vlm_model, image, given_instruction, sleep=DEFAULT_SLEEP):
    ...
    try:
        responses = vlm_model.generate_content(
            [image, prompt_analyze],
...
def query_vlm_for_rating_given_summarization(vlm_model, trajectory_analysis, given_instruction, max_rating):
    ...
    try:
        responses = vlm_model.generate_content(
            prompt_conclusion,
            generation_config=genai.GenerationConfig(temperature=0, response_mime_type="text/x.enum", response_schema=Choice)
        )
        responses.resolve()
    except Exception as error:
        return None, [], error
    ...
    gemini_analysis = responses.text
    ...
    while (start < trajectory_len and not error_duing_query_vlm):
        ...
        while not query_success:
            ...
            if error is None:
                ...
                if isinstance(error, BrokenResponseError):
